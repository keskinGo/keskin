...
class Ticket(scrapy.Spider):
    ...
    def parse(self, response):
        # 春秋航空
        print('response信息：', response.text)
        if response.meta['source_code'] == 1:
            ...
            if ticket_infos:
                for ticket_info in ticket_infos:
                    ...
                    # item['search_time'] = constant.NOW_STR_TIME_SS  # 查询价格的时间（yyyy-mm-dd hh:mi:ss）
                    ...
                    # print('*********************************', item)
                    logging.info('始发站：%s，终点站：%s，航班号：%s，出发时间：%s', ticket_info['Departure'], ticket_info['Arrival'],
                                 ticket_info['No'], ticket_info['DepartureTime'])
                    yield item
            else:
                pass
